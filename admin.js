...
const el = {
  ...
  syncShopAnalytics: document.querySelector("#syncShopAnalytics"),
  ...
  shopAnalyticsTitle: document.querySelector("#shopAnalyticsTitle"),
  shopAnalyticsSummary: document.querySelector("#shopAnalyticsSummary"),
  shopAnalyticsSignal: document.querySelector("#shopAnalyticsSignal"),
  shopAnalyticsKpis: document.querySelector("#shopAnalyticsKpis"),
  shopAnalyticsActions: document.querySelector("#shopAnalyticsActions"),
...
const i18n = {
  zh: {
    ...
    analyticsTitle: "Shop Analytics",
    analyticsEmpty: "同步 TikTok Shop Analytics 后，查看点击、加购、转化和商品动作建议。",
    analyticsWaiting: "待同步",
    analyticsReady: "已读取",
    analyticsNoData: "暂未读取到商品表现数据。需要开启 TikTok Shop Analytics 权限，重新授权后再同步。",
    analyticsSummary: (count, date) => `已读取 ${count} 个商品表现；数据日期：${date}。`,
    analyticsTraffic: "流量",
    analyticsCart: "加购",
    analyticsConversion: "转化",
    analyticsGmv: "GMV",
    analyticsActionTitle: "商品动作建议",
    promoTitle: "产品促销组合建议",
  ...
  en: {
    ...
    analyticsTitle: "Shop Analytics",
    analyticsEmpty: "Sync TikTok Shop Analytics to view clicks, carts, conversion, and product actions.",
    analyticsWaiting: "Waiting",
    analyticsReady: "Loaded",
    analyticsNoData: "No product performance data loaded yet. Enable TikTok Shop Analytics, re-authorize, then sync again.",
    analyticsSummary: (count, date) => `${count} product performance rows loaded; date range: ${date}.`,
    analyticsTraffic: "Traffic",
    analyticsCart: "Cart",
    analyticsConversion: "Conversion",
    analyticsGmv: "GMV",
    analyticsActionTitle: "Product Action Queue",
    ...
    syncingAnalytics: "Syncing Analytics...",
  },
};
...
function applyLanguage() {
  ...
  el.shopAnalyticsTitle.textContent = t("analyticsTitle");
  ...
  el.syncShopAnalytics.textContent = t("syncAnalytics");
  ...
  renderShopAnalytics(state.tiktok?.analytics || null, state.tiktok?.products || []);
...
function renderTikTokConnection(connection = state.tiktok) {
  ...
  el.syncShopAnalytics.disabled = !tiktok.authorized;
  ...
  renderShopAnalytics(tiktok.analytics || null, tiktok.products || []);
...
function analyticsAction(product) {
  const ctr = Number(product.ctr || 0);
  const cartRate = Number(product.addToCartRate || 0);
  const conversion = Number(product.conversionRate || 0);
  const orders = Number(product.orders || 0);
  if (ctr >= 2 && cartRate >= 6 && conversion < 1) {
    return currentLanguage === "zh" ? "加购强但成交弱：优先做券、包邮门槛和详情页信任点。" : "Strong cart, weak checkout: add voucher, free-shipping threshold, and trust details.";
  }
  if (ctr >= 2 && cartRate < 4) {
    return currentLanguage === "zh" ? "点击不错但不加购：检查价格、主图承诺和首屏卖点是否一致。" : "Clicks but weak carts: check price, image promise, and above-fold message.";
  }
  if (ctr < 1 && orders > 0) {
    return currentLanguage === "zh" ? "转化能跑但流量弱：适合换封面、标题关键词或加短视频曝光。" : "Converts but lacks traffic: refresh cover, title keywords, or push video exposure.";
  }
  if (conversion >= 2) {
    return currentLanguage === "zh" ? "转化健康：保留主推位，检查库存并考虑小额加购组合。" : "Healthy conversion: keep it featured, check stock, and pair with add-ons.";
  }
  return currentLanguage === "zh" ? "数据偏弱：先优化素材和标题，不建议马上加大折扣。" : "Weak signal: optimize creative and title before adding deeper discounts.";
}

function renderShopAnalytics(analytics, products = []) {
  if (!el.shopAnalyticsKpis) return;
  const rows = Array.isArray(analytics?.products) ? analytics.products : [];
  if (!rows.length) {
    el.shopAnalyticsSignal.textContent = t("analyticsWaiting");
    el.shopAnalyticsSummary.textContent = analytics?.source ? t("analyticsNoData") : t("analyticsEmpty");
    el.shopAnalyticsKpis.innerHTML = `
      <article><span>${t("analyticsTraffic")}</span><strong>-</strong><small>Views / Clicks / CTR</small></article>
      <article><span>${t("analyticsCart")}</span><strong>-</strong><small>Add-to-cart rate</small></article>
      <article><span>${t("analyticsConversion")}</span><strong>-</strong><small>Orders / CVR</small></article>
      <article><span>${t("analyticsGmv")}</span><strong>-</strong><small>GMV / Units sold</small></article>
    `;
    el.shopAnalyticsActions.innerHTML = `
      <div class="risk-empty">
        <strong>${t("analyticsActionTitle")}</strong>
        <span>${t("analyticsNoData")}</span>
      </div>
    `;
    return;
  }
  const totals = analytics.totals || {};
  const dateRange = [analytics.startDate, analytics.endDate].filter(Boolean).join(" - ");
  el.shopAnalyticsSignal.textContent = t("analyticsReady");
  el.shopAnalyticsSummary.textContent = t("analyticsSummary", rows.length, dateRange || "-");
  el.shopAnalyticsKpis.innerHTML = `
    <article><span>${t("analyticsTraffic")}</span><strong>${compactNumber(totals.clicks)}</strong><small>${compactNumber(totals.views)} views · CTR ${percent(totals.ctr)}</small></article>
    <article><span>${t("analyticsCart")}</span><strong>${compactNumber(totals.addToCarts)}</strong><small>${percent(totals.addToCartRate)} add-to-cart rate</small></article>
    <article><span>${t("analyticsConversion")}</span><strong>${compactNumber(totals.orders)}</strong><small>${percent(totals.conversionRate)} conversion · ${compactNumber(totals.units)} units</small></article>
    <article><span>${t("analyticsGmv")}</span><strong>${compactNumber(totals.gmv)}</strong><small>${escapeHtml(analytics.currency || "LOCAL")}</small></article>
  `;
  const productMap = new Map(products.map((product) => [String(product.productId), product]));
  const sorted = rows
    .map((row) => ({ ...row, catalog: productMap.get(String(row.productId)) || null }))
    .sort((a, b) => Number(b.gmv || 0) - Number(a.gmv || 0))
    .slice(0, 8);
  el.shopAnalyticsActions.innerHTML = `
    <h4>${t("analyticsActionTitle")}</h4>
    <div class="analytics-product-list">
      ${sorted.map((product) => `
        <div class="analytics-product-row">
          ${product.imageUrl || product.catalog?.imageUrl ? `<img src="${escapeAttr(product.imageUrl || product.catalog.imageUrl)}" alt="${escapeAttr(product.title)}" loading="lazy" referrerpolicy="no-referrer" />` : `<div class="shop-product-image-empty">No image</div>`}
          <div>
            <strong>${escapeHtml(shortTitle(product.title || product.catalog?.title || "Untitled product"))}</strong>
            <span>CTR ${percent(product.ctr)} · ATC ${percent(product.addToCartRate)} · CVR ${percent(product.conversionRate)} · GMV ${compactNumber(product.gmv)}</span>
            <small>${escapeHtml(analyticsAction(product))}</small>
          </div>
        </div>
      `).join("")}
    </div>
...
async function syncShopAnalytics() {
  el.syncShopAnalytics.disabled = true;
  el.syncShopAnalytics.textContent = t("syncingAnalytics");
  try {
    const response = await fetch("/api/admin/tiktok/analytics", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ limit: 50, currency: "LOCAL" }),
    });
    const payload = await response.json();
    if (!response.ok || payload.error) throw new Error(payload.error || "Shop Analytics 同步失败");
    state.tiktok = payload;
    renderTikTokConnection(payload);
  } finally {
    el.syncShopAnalytics.disabled = false;
    el.syncShopAnalytics.textContent = t("syncAnalytics");
  }
}
...
el.syncShopAnalytics.addEventListener("click", async () => {
  try {
    await syncShopAnalytics();
  } catch (error) {
    alert(`Shop Analytics 同步失败：${error.message}`);
    await loadTikTokConnection().catch(() => {});
  }
});
